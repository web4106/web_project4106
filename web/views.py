...
def register(request):
    ...
    if request.method == 'POST':
        ...
        form = Register()
    return render(request, 'web/Register.html', {'form': form,'code_err':code_err})
...
def login(request):
    if request.method == 'POST':
        ...
        if user is not None and user.is_active:
            ...
            return render(request,'web/Login.html',{'error':'用户名和密码不匹配！'})
...
def push_goods(request):
    ...
    return render(request, 'web/PushGoods.html',{'form':f,'code_err':code_err})
...
def look_goods(request,number):
    ...
    if(goods):
        ...
        f = GoodsWordsForm()
        return render(request,'web/GoodsMessage.html',{'uid':user.id,'words':words,'form':f,'profile':profile,'goods':goods,'image':image})
    else:
        raise Http404()
...
def goods_list(request):
    ...
    if not goods:
        return render(request, "web/No_Goods.html")
    ...
    return render(request,'web/GoodsList.html',{'goods':goods,'form':form,'page':pg})
...
def look_user(request,id):
    ...
    return render(request,'web/UserMessage.html', {'profile':profile,'goods': goods})
...
def user_message(request):
    ...
    return render(request,'web/MyMessage.html', {'log':log,'profile': profile, 'goods': goods})
...
def edit_goods(request,id):
    ...
    return render(request,'web/Edit.html',{'type':type,'form':form,'image':pic,'id':goods.id})
...
def show_log(request):
    ...
    return render(request,"web/MyLog.html",{'Newlog':Newlog,'Oldlog':Oldlog})
...
def change_myself(request):
    ...
    return render(request,'web/ChangeMyself.html',
...
def change_passwd(request):
    if request.method != 'POST':
        ...
        if not user:
            ...
            return render(request, 'web/ChangeMyself.html', {'profile': profile,
                                                                   'form': form,
                                                                   'user': request.user,
        ...
        if pass_form.is_valid():
            ...
            return render(request, "web/doing_success.html", {'mes': '修改密码'})
    return render(request, 'web/ChangeMyself.html', {'profile': profile,
                                                           'form': form,
                                                           'user': request.user,
                                                           'email_form': email_form,
...
def find_passwd(request):
    ...
    return render(request,'web/ForgetPasswd.html',{'form':form})
...
def reset_passwd(request):
    if request.method == 'GET':
        ...
        if profile and (datetime.datetime.now()-profile.PasswdCodeTime).seconds <= 3600*24:
            ...
            return render(request,"web/ResetPasswd.html",{'form':form})
        else:
        ...
        if profile and (datetime.datetime.now() - profile.PasswdCodeTime).seconds <= 3600 * 24:
            ...
            if form.is_valid():
                ...
                profile.save()
                return render(request, "web/doing_success.html", {'mes': '重置密码'})
            else:
                return render(request,'web/ResetPasswd.html',{'form':form})
        else:
            return HttpResponse("链接已过期或不存在！")
